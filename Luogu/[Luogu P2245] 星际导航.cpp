...
void lca(int x, int y)
{
    ...
    for (int i = 20; i >= 0; i--)
        if (fa[x][i] != fa[y][i])
        {
            ans = std::max(ans, std::max(dis[x][i], dis[y][i]));
            x = fa[x][i], y = fa[y][i];
        }
    ans = std::max(ans, std::max(dis[x][0], dis[y][0]));
    printf("%d\n", ans);
}
...
int main()
{
    ...
    for (int i = 1; i <= m; i++)
        if (!vis[i])
            depth[i] = 1, dfs(i), fa[i][0] = i;
    for (int i = 1; i <= 20; i++)
        for (int j = 1; j <= m; j++)
        {
            fa[j][i] = fa[fa[j][i - 1]][i - 1];
            dis[j][i] = std::max(dis[j][i - 1], dis[fa[j][i - 1]][i - 1]);
        }
    scanf("%d", &k);
    for (int u, v; k; k--)
    {
