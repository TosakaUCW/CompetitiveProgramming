    ...
    for (int i = 1; i <= lena; i++)
        for (int j = 1; j <= lenb; j++)
            for (int k = 1; k <= lenc; k++)
                ...
                {
                    f[i][j][k] = (f[i - 1][j - 1][k - 1] * 2 + 1) % P;
                    if (lasta[i][a[i]] and lastb[j][b[j]] and lastc[k][c[k]])
                        (f[i][j][k] -= (f[lasta[i][a[i]] - 1][lastb[j][b[j]] - 1][lastc[k][c[k]] - 1] + 1) % P) %= P;
                }
                else
                {
                    f[i][j][k] =
                        ((f[i - 1][j][k] + f[i][j - 1][k] + f[i][j][k - 1]) % P -
                         f[i - 1][j - 1][k] - f[i][j - 1][k - 1] - f[i - 1][j][k - 1] +
                         f[i - 1][j - 1][k - 1]) %
                        P;
                }
    printf("%lld", (f[lena][lenb][lenc] + P) % P);
    return 0;
}