...
struct Poly : public std::vector<MInt<P>> {
    ...
    constexpr Poly pow(int k1, int k2, int k3, int m) const {
        if (k3 == 0) {
            auto f = Poly(m);
            f[0] = 1;
            return f;
...
signed main() {
    // freopen("P5273_5.in", "r", stdin);
    // freopen("P5273_5.ans", "w", stdout);
